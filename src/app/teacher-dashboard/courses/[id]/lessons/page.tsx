        ...
        <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl gradient-primary text-white text-xs sm:text-sm font-bold shadow hover:opacity-90 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:opacity-40"
              style={{ fontFamily: font }}
            >
              {permissions.canEditContent ? <Plus size={15} /> : <Lock size={15} />}
              {t.addUnit}
            </button>
          </div>
        </div>
...
function UnitCard({
  ...
  const guard = (allowed: boolean) => {
    if (!allowed) toast.error(t.noPermission);
    return allowed;
  };

  const reorderUnit = async (dir: 'up' | 'down') => {
    if (!guard(permissions.canReorder)) return;
    await fetch('/api/admin/units', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const openAddVideo = () => {
    if (!guard(permissions.canAddVideo)) return;
    setVideoModal({ mode: 'add' });
  };
  const openEditVideo = (lesson: Lesson) => {
    if (!guard(permissions.canEditContent)) return;
    setVideoModal({ mode: 'edit', lesson });
  };
  const openAddQuiz = () => {
    if (!guard(permissions.canAddExam)) return;
    setAddQuizModal(true);
  };
  const openManageLesson = (lesson: Lesson) => {
    if (!guard(permissions.canEditContent)) return;
      ...
      <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 sm:py-4 border-b border-border bg-muted/20 flex-wrap">
        <div className="flex flex-col gap-0.5 flex-shrink-0">
          <button
            ...
            disabled={isFirst || !permissions.canReorder}
          ...
          <button
            ...
            disabled={isLast || !permissions.canReorder}
            title={!permissions.canReorder ? t.noPermission : undefined}
            className="p-0.5 rounded text-muted-foreground hover:text-foreground disabled:opacity-20"
          >
            <ChevronDown size={13} />
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg border border-border text-[10px] sm:text-xs font-bold text-primary hover:bg-primary/10 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
            style={{ fontFamily: font }}
          >
            {permissions.canAddVideo ? <Video size={12} /> : <Lock size={12} />} {t.addVideo}
          </button>
          <button
            onClick={openAddQuiz}
            disabled={!permissions.canAddExam}
            title={!permissions.canAddExam ? t.noPermission : undefined}
            className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg border border-border text-[10px] sm:text-xs font-bold text-accent hover:bg-accent/10 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
            style={{ fontFamily: font }}
          >
            {permissions.canAddExam ? <ClipboardList size={12} /> : <Lock size={12} />} {t.addQuiz}
          </button>
            ...
            >
              <div className="flex flex-col gap-0.5 flex-shrink-0">
                ...
                <button
                  ...
                  disabled={idx === lessons.length - 1 || !permissions.canReorder}
                  title={!permissions.canReorder ? t.noPermission : undefined}
                  className="p-0.5 rounded text-muted-foreground hover:text-foreground disabled:opacity-20"
                >
                  <ChevronDown size={12} />
              ...
              <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
                {lesson.type === 'exam' ? (
                  <button
                    onClick={() => openManageLesson(lesson)}
                    title={permissions.canEditContent ? t.manage : t.noPermission}
                    disabled={!permissions.canEditContent}
                    className="flex items-center gap-1 p-1 sm:p-1.5 rounded-lg text-muted-foreground hover:bg-secondary/10 hover:text-secondary transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-muted-foreground"
                  >
                    <Settings size={13} />
                  </button>
                ) : (
...
// Note: opening this modal is already permission-gated by the parent,
// so no additional checks are needed inside.
// ═══════════════════════════════════════════════════════════════
function VideoModal({
  courseId,
  unitId,
          ...
          <div className="flex flex-col gap-2.5 sm:gap-3">
            ...
            <div className="flex justify-end">
              <button
                ...
                disabled={savingSettings || !permissions.canEditContent}
                title={!permissions.canEditContent ? t.noPermission : undefined}
                ...
                >
                  <div className="flex flex-col gap-0.5 flex-shrink-0">
                    ...
                    <button
                      ...
                      disabled={idx === visibleExamQuestions.length - 1 || !permissions.canReorder}
                      title={!permissions.canReorder ? t.noPermission : undefined}
                      className="p-0.5 text-muted-foreground hover:text-foreground disabled:opacity-20"
                    >
                      <ChevronDown size={11} />
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <input
                      ...
                      className="w-12 sm:w-14 text-[10px] sm:text-xs text-center py-1 rounded-lg border border-border bg-background text-foreground outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50"
                      title={t.markLabel}
                    />
                    <span className="text-[10px] sm:text-xs text-muted-foreground">
                      {t.markUnit}
